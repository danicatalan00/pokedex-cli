...
def _frames_composited(grid: list[list[Cell]], caught: bool, rng, style: BallAnimationStyle):
...
def generate_capture_frames(
    ...
    *,
    ...
    """Generate capture frames without terminal, subprocess, network, or storage."""
    random_source = rng or random
    style = _ball_style(ball_slug)
    grid = _parse_sprite(sprite_ansi) if sprite_ansi else []
    frames = (
        _frames_composited(grid, caught, random_source, style)
        if grid and grid[0]
        else _frames_fallback(caught, random_source, style)
    )
    return list(frames)


def play_capture_animation(
    console: Console,
    species: str,
    form: str,
    shiny: bool,
...
def generate_evolution_frames(
    old_sprite_ansi: str, new_sprite_ansi: str, *, speed: float = 1.0
) -> list[tuple[Text, float]]:
    """Generate evolution frames from prepared sprites without external effects."""
    old_grid = _parse_sprite(old_sprite_ansi)
    new_grid = _parse_sprite(new_sprite_ansi)
    if not old_grid or not new_grid:
        return []
    return list(_evolution_frames(old_grid, new_grid, speed))


def play_evolution_animation(
    console: Console,
    old_species: str,
    old_form: str,
    new_species: str,
    ...
    try:
        ...
        new_sprite = (
            sprite_renderer.capture_sprite(new_species, new_form, shiny)
            if sprite_renderer
            else None
        )
        if not old_sprite or not new_sprite:
            raise RuntimeError("sprite no disponible")
        frames = generate_evolution_frames(old_sprite, new_sprite, speed=speed)
        with Live(console=console, refresh_per_second=30, transient=True) as live:
            for renderable, delay in frames:
                live.update(Align.center(renderable))
                time.sleep(delay)
