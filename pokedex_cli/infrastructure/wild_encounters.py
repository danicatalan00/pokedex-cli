...
def _ensure_pokemon_db() -> list[dict[str, Any]] | None:
    ...
    try:
        payload = json.loads(KRABBY_POKEMON_JSON.read_text())
...
def pick_species_form_shiny(generations: str) -> tuple[str, str, bool]:
    ...
    base_pool = list_pool(generations)
    return random.choice(base_pool), "regular", shiny
...
def capture_sprite(species: str, form: str, shiny: bool) -> str | None:
    ...
    está instalado."""
    return KrabbyClient(pokemon_json_path=KRABBY_POKEMON_JSON).capture_sprite(species, form, shiny)
